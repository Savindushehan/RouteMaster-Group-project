...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String Viewinquiry=request.getParameter("viewinquiry");
		...
		}else if("Viewinquiry".equals(Viewinquiry)) {
			String dynamicValue6=request.getParameter("RID");
			request.setAttribute("attr3", dynamicValue6);
			
			
			 try {
			        List<Inquiry> regDetails = RegisteruserDButy.validateInquiry(dynamicValue6);
			        request.setAttribute("regDetails", regDetails);
